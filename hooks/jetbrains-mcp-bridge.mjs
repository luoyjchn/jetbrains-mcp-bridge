import { evaluate, extractPathFromCommand, loadConfig } from '../src/core.mjs';
...
async function main() {
    ...
    const configSource = [];
    try {
        if (existsSync(resolve(pluginRoot, 'config', 'default-global.json5'))) configSource.push('plugin-default');
    } catch {
    }
    try {
        if (existsSync(resolve(homedir(), '.claude', 'jetbrains-mcp-bridge.json5'))) configSource.push('user-global');
    } catch {
    }
    if (projectDir) {
        try {
            if (existsSync(resolve(projectDir, '.claude', 'jetbrains-mcp-bridge.json5'))) configSource.push('project');
        } catch {
        }
    }
    writeLog({
    ...
    if (!cache) {
        ...
        writeLog({
            ...
            servers: Object.entries(status).map(([k, v]) => `${k}=${v}`)
        }, config.debug);
    } else {
        ...
        writeLog({
            ...
            step: 'HOOK-MCP',
            source: 'cache',
            serverCount: Object.keys(cache.status || {}).length
        }, config.debug);
    }
    ...
    if (result.action === 'block') {
        ...
        let message = `检测到${result.reason}操作。请改用 JetBrains IDE 的 ${mcpToolName} 工具，它使用 IDE 索引更准确高效。`;
